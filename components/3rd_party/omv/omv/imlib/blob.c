...
void imlib_find_blobs(list_t *out, image_t *ptr, rectangle_t *roi, unsigned int x_stride, unsigned int y_stride,
                ...
                for (int y = roi->y, yy = roi->y + roi->h, y_max = yy - 1; y < yy; y += y_stride) {
                    ...
                    for (int x = roi->x + (y % x_stride), xx = roi->x + roi->w, x_max = xx - 1; x < xx; x += x_stride) {
                        ...
                        && COLOR_THRESHOLD_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x), &lnk_data, invert)) {
                            ...
                            for(;;) {
                                int left = x, right = x;
                                pixel_rgb_t *row = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(ptr, y);
                                uint32_t *bmp_row = IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(&bmp, y);
    ...
    if (bmp.data) fb_free(bmp.data); // bitmap
...
void imlib_flood_fill_int(image_t *out, image_t *img, int x, int y,
    ...
    switch (img->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            pixel_rgb_t rgb_seed_threshold = COLOR_R8_G8_B8_TO_RGB888(seed_threshold & 0xff, (seed_threshold >> 8) & 0xff, (seed_threshold >> 16) & 0xff);
            pixel_rgb_t rgb_floating_threshold = COLOR_R8_G8_B8_TO_RGB888(floating_threshold & 0xff, (floating_threshold >> 8) & 0xff, (floating_threshold >> 16) & 0xff);
            for(pixel_rgb_t seed_pixel = IMAGE_GET_RGB888_PIXEL(img, x, y);;) {
                int left = x, right = x;
                pixel_rgb_t *row = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                uint32_t *out_row = IMAGE_COMPUTE_BINARY_PIXEL_ROW_PTR(out, y);

                while ((left > 0)
                && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, left - 1))
                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, left - 1), seed_pixel, rgb_seed_threshold)
                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, left - 1),
                                      IMAGE_GET_RGB888_PIXEL_FAST(row, left), rgb_floating_threshold)) {
                    left--;
                }

                while ((right < (img->w - 1))
                && (!IMAGE_GET_BINARY_PIXEL_FAST(out_row, right + 1))
                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, right + 1), seed_pixel, rgb_seed_threshold)
                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, right + 1),
                                      IMAGE_GET_RGB888_PIXEL_FAST(row, right), rgb_floating_threshold)) {
                    right++;
                }
                ...
                for(;;) {
                    if (lifo_size(&lifo) < lifo_len) {
                        pixel_rgb_t *old_row = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                        ...
                        if (y > 0) {
                            ...
                            for (int i = top_left; i <= right; i++) {
                                if ((!IMAGE_GET_BINARY_PIXEL_FAST(out_row, i))
                                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, i), seed_pixel, rgb_seed_threshold)
                                && COLOR_BOUND_RGB888(IMAGE_GET_RGB888_PIXEL_FAST(row, i),
                                                      IMAGE_GET_RGB888_PIXEL_FAST(old_row, i), rgb_floating_threshold)) {
                                    xylr_t context;
                                    context.x = x;
                                    context.y = y;
                                    context.l = left;
