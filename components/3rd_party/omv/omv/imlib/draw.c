...
void imlib_draw_row_teardown(imlib_draw_row_data_t *data) {
    if (data->smuad_alpha_palette) {
        fb_free(data->smuad_alpha_palette);
        data->smuad_alpha_palette = NULL;
    }
    #ifdef IMLIB_ENABLE_DMA2D
    if (data->dma2d_initialized) {
        ...
        if (data->src_img_pixfmt == PIXFORMAT_GRAYSCALE) {
            #error "fb_free() is commented, but when using dma2d we need to free the memory of clut here."
            // fb_free();                                              // clut...
        }
       if (data->row_buffer[1]) {
            fb_free(data->row_buffer[1]); // data->row_buffer[1]
            data->row_buffer[1] = NULL;
       }
    }
    #endif
    if (data->row_buffer[0]) {
        fb_free(data->row_buffer[0]); // data->row_buffer[0]
        data->row_buffer[0] = NULL;
    }
}
...
void imlib_draw_image(image_t *dst_img,
    ...
    if (hint & IMAGE_HINT_TRANSPOSE) {
        ...
        if(data) fb_free(data); // fb_alloc_all

        if (t_src_img.data != src_img->data) {
            if (t_src_img.data) fb_free(t_src_img.data);
        }

        if (&new_src_img == src_img) {
            if (new_src_img.data) fb_free(new_src_img.data);
        }
    ...
    if ((0 <= x) && (x < img->w) && (0 <= y) && (y < img->h)) {
        ...
        switch (img->pixfmt) {
            ...
            case PIXFORMAT_RGB888: {
                color_seed_threshold = ((fast_floorf(seed_threshold * COLOR_R8_MAX)) & 0xff)
                                    | ((fast_floorf(seed_threshold * COLOR_G8_MAX) << 8) & 0xff00)
                                    | ((fast_floorf(seed_threshold * COLOR_B8_MAX) << 16) & 0xff0000);
                color_floating_threshold = ((fast_floorf(floating_threshold * COLOR_R8_MAX)) & 0xff)
                                        | ((fast_floorf(floating_threshold * COLOR_G8_MAX) << 8) & 0xff00)
                                        | ((fast_floorf(floating_threshold * COLOR_B8_MAX) << 16) & 0xff0000);
                break;
            }
            default: {
                break;
        ...
        switch (img->pixfmt) {
            ...
            case PIXFORMAT_RGB888: {
                pixel_rgb_t rgb_c = COLOR_R8_G8_B8_TO_RGB888(c & 0xff, (c >> 8) & 0xff, (c >> 16) & 0xff);
                ...
                for (int y = 0, yy = out.h; y < yy; y++) {
                    ...
                    for (int x = 0, xx = out.w; x < xx; x++) {
                        if (IMAGE_GET_BINARY_PIXEL_FAST(out_row_ptr, x) ^ invert) {
                            IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, x, rgb_c);
                        } else if (clear_background) {
