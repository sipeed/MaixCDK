...
size_t image_line_size(image_t *ptr) {
    switch (ptr->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            return IMAGE_RGB888_LINE_LEN_BYTES(ptr);
        }
        default: {
            return 0;
        }
...
void imlib_rgb888_to_lab(pixel_rgb_t pixel, uint8_t *l, int8_t *a, int8_t *b)
{
    ...
    *l = IM_MAX(IM_MIN(fast_floorf(116 * y) - 16, COLOR_L_MAX), COLOR_L_MIN);
    *a = IM_MAX(IM_MIN(fast_floorf(500 * (x - y)), COLOR_A_MAX), COLOR_A_MIN);
    *b = IM_MAX(IM_MIN(fast_floorf(200 * (y - z)), COLOR_B_MAX), COLOR_B_MIN);
}
...
void imlib_image_operation(image_t *img, const char *path, image_t *other, int scalar, line_op_t op, void *data) {
    if (path) {
        ...
        switch (img->pixfmt) {
            ...
            case PIXFORMAT_RGB888: {
                pixel_rgb_t *row_ptr = fb_alloc(IMAGE_RGB888_LINE_LEN_BYTES(img), FB_ALLOC_NO_HINT);
                pixel_rgb_t scalar_value = {.r = scalar, .g = scalar, .b = scalar};
                for (int i=0, ii=img->w; i<ii; i++) {
                    IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, i, scalar_value);
                }
...
void imlib_save_image(image_t *img, const char *path, rectangle_t *roi, int quality) {
    switch (imblib_parse_extension(img, path)) {
        ...
        case FORMAT_DONT_CARE:
            ...
            if (IM_IS_JPEG(img)) {
                char *alloc = (char *)fb_alloc(strlen(path) + 5, FB_ALLOC_NO_HINT);
                char *new_path = strcat(strcpy(alloc, path), ".jpg");
                jpeg_write(img, new_path, quality);
                if (alloc) fb_free(alloc);
            } else if (img->pixfmt == PIXFORMAT_PNG) {
                char *alloc = (char *)fb_alloc(strlen(path) + 5, FB_ALLOC_NO_HINT);
                char *new_path = strcat(strcpy(, path), ".png");
                png_write(img, new_path);
                if (alloc) fb_free(alloc);
            } else if (IM_IS_BAYER(img)) {
                FIL fp;
                char *alloc = (char *)fb_alloc(strlen(path) + 5, FB_ALLOC_NO_HINT);
                char *new_path = strcat(strcpy(alloc, path), ".raw");
                ...
                char *alloc = (char *)fb_alloc(strlen(path) + 5, FB_ALLOC_NO_HINT);
                char *new_path = strcat(strcpy(alloc, path), ".bmp");
...
void imlib_zero(image_t *img, image_t *mask, bool invert) {
    switch (img->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            pixel_rgb_t rgb_zero = COLOR_R8_G8_B8_TO_RGB888(0, 0, 0);
            for (int y = 0, yy = img->h; y < yy; y++) {
                pixel_rgb_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                for (int x = 0, xx = img->w; x < xx; x++) {
                    if (image_get_mask_pixel(mask, x, y) ^ invert) {
                        IMAGE_PUT_RGB888_PIXEL_FAST(row_ptr, x, rgb_zero);
                    }
                }
            }
            break;
...
void imlib_lens_corr(image_t *img, float strength, float zoom, float x_corr, float y_corr) {
    ...
    switch (img->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            ...
            for (int y = 0; y < halfHeight; y++) {
                pixel_rgb_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, y);
                pixel_rgb_t *row_ptr2 = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(img, h-1-y);
                int newY = y - halfHeight;
                int newY2 = newY * newY;
                ...
                for (int x = 0; x < halfWidth; x++) {
                    ...
                    // top 2 pixels
                    if (sourceY_down >= 0 && sourceY_down < h) {
                        pixel_rgb_t *ptr = tmp + (w * sourceY_down);
                    ...
                    // bottom 2 pixels
                    if (sourceY_up >= 0 && sourceY_up < h) {
                        pixel_rgb_t *ptr = tmp + (w * sourceY_up);
    ...
    if (precalculated_table) fb_free(precalculated_table);  // precalculated_table
    if (data) fb_free(data);                                // data
}
...
int imlib_image_mean(image_t *src, int *r_mean, int *g_mean, int *b_mean) {
    ...
    switch (src->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            for (int i=0; i<n; i++) {
                pixel_rgb_t p = ((pixel_rgb_t*)src->pixels)[i];
