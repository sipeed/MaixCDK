...
bool jpeg_compress(image_t *src, image_t *dst, int quality, bool realloc) {
        ...
        if (!y_offset) {
            ...
            while (!JPEG_input_paused) {
                ...
                if (JPEG_output_paused) {
                    ...
                    if (mcu_row_buffer) fb_free(mcu_row_buffer); // mcu_row_buffer (after DMA is aborted)
                    return true; // overflow
                }
            }
    ...
    if (mcu_row_buffer) fb_free(mcu_row_buffer); // mcu_row_buffer
...
void jpeg_write(image_t *img, const char *path, int quality) {
    ...
    if (IM_IS_JPEG(img)) {
        ...
        if (out.pixels) fb_free(out.pixels); // frees alloc in jpeg_compress()
    }
    file_close(&fp);
}
