        ...
        case PIXFORMAT_RGB888: {
            for (int y = 0, yy = img->h; y < yy; y++) {
                ...
                for (int x = 0, xx = img->w; x < xx; x++) {
                    ...
                    COLOR_RGB888_TO_R8(dataPixel) = ~COLOR_RGB888_TO_R8(dataPixel);
                    COLOR_RGB888_TO_G8(dataPixel) = ~COLOR_RGB888_TO_G8(dataPixel);
                    COLOR_RGB888_TO_B8(dataPixel) = ~COLOR_RGB888_TO_B8(dataPixel);
    ...
    switch (img->pixfmt) {
        ...
        case PIXFORMAT_RGB888: {
            ...
            for (int i = 0, j = img->w; i < j; i++) {
                ...
                if ((!mask) || image_get_mask_pixel(mask, h_i, v_line)) {
                    pixel_rgb_t pixel = IMAGE_GET_RGB888_PIXEL_FAST(((pixel_rgb_t *) other), h_i);
...
//                    p = IM_MIN(p, COLOR_BINARY_MAX);
        ...
        case PIXFORMAT_RGB888: {
            ...
            for (int i = 0, j = img->w; i < j; i++) {
                if ((!mask) || image_get_mask_pixel(mask, i, line)) {
                    pixel_rgb_t dataPixel = IMAGE_GET_RGB888_PIXEL_FAST(data, i);
                    pixel_rgb_t otherPixel = IMAGE_GET_RGB888_PIXEL_FAST(((pixel_rgb_t *) other), i);
