                                  ...
                                  struct quirc_data *data)
{
    ...
    if ((code->size - 17) % 4)
        { if (ds) fb_free(ds); return QUIRC_ERROR_INVALID_GRID_SIZE; }
    ...
    if (data->version < 1 ||
        ...
        { if (ds) fb_free(ds); return QUIRC_ERROR_INVALID_VERSION; }
...
void imlib_find_qrcodes(list_t *out, image_t *ptr, rectangle_t *roi)
{
    ...
    image_t img;
    int need_free_controller_image = 1;
    if (roi->w == ptr->w && roi->h == ptr->h && ptr->pixfmt == PIXFORMAT_GRAYSCALE) {
        controller->image = ptr->data;
        need_free_controller_image = 0;
    } else {
        img.w = roi->w;
        img.h = roi->h;
    ...
    for (int i = 0, j = quirc_count(controller); i < j; i++) {
        ...
        if (code) fb_free(code);
    }

    if (!need_free_controller_image) {
