...
void imlib_get_similarity(image_t *img,
    ...
    *max = state.similarity_max;
    ...
    if (state.sum2Buckets) fb_free(state.sum2Buckets);
    if (state.sum2BucketsOfY) fb_free(state.sum2BucketsOfY);
    if (state.sum2BucketsOfX) fb_free(state.sum2BucketsOfX);
    if (state.sumBucketsOfY) fb_free(state.sumBucketsOfY);
    if (state.sumBucketsOfX) fb_free(state.sumBucketsOfX);
}
        ...
        case PIXFORMAT_RGB888: {
            ...
            if ((!thresholds) || (!list_size(thresholds))) {
                ...
                if (!other) {
                    ...
                    for (int y = roi->y, yy = roi->y + roi->h; y < yy; y++) {
                        pixel_rgb_t *row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(ptr, y), *other_row_ptr = IMAGE_COMPUTE_RGB888_PIXEL_ROW_PTR(other, y);
                        for (int x = roi->x, xx = roi->x + roi->w; x < xx; x++) {
                            pixel_rgb_t pixel = IMAGE_GET_RGB888_PIXEL_FAST(row_ptr, x), other_pixel = IMAGE_GET_RGB888_PIXEL_FAST(other_row_ptr, x);
...
bool imlib_get_regression(find_lines_list_lnk_data_t *out,
    ...
    if (!robust) {
        ...
        if (points) fb_free(points);                        // points
        if (y_delta_histogram) fb_free(y_delta_histogram);  // y_delta_histogram
        if (x_delta_histogram) fb_free(x_delta_histogram);  // x_delta_histogram
        if (y_histogram) fb_free(y_histogram);              // y_histogram
        if (x_histogram) fb_free(x_histogram);              // x_histogram
    }
