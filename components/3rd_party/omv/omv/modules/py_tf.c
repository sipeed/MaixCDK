...
STATIC mp_obj_t int_py_tf_load(mp_obj_t path_obj, bool alloc_mode, bool helper_mode) {
    ...
    if (tensor_arena) fb_free(tensor_arena); // free fb_alloc_all()

    if (!helper_mode) {
        if (py_tf_putchar_buffer) {
            fb_free(py_tf_putchar_buffer); // free py_tf_alloc_putchar_buffer()
            py_tf_putchar_buffer = NULL;
        }
    }
        ...
        for (int y = roi.y + ((y_overlap != -1.0f)
            ...
            for (int x = roi.x + ((x_overlap != -1.0f)
                ...
                if (rectangle_overlap(&roi, &new_roi)) {
                    ...
                    py_tf_input_data_callback_data.img = image;
...
STATIC mp_obj_t int_py_tf_segment(bool detecting_mode, uint n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    enum { ARG_roi, ARG_thresholds, ARG_invert };
    static const mp_arg_t allowed_args[] = {
        { MP_QSTR_roi, MP_ARG_OBJ | MP_ARG_KW_ONLY, {.u_rom_obj = MP_ROM_NONE} },
    ...
    for (int i = 0, ii = img_list->len; i < ii; i++) {
        ...
        for (int j = 0, jj = list_size(&out); j < jj; j++) {
            ...
            if (hist.LBins) fb_free(hist.LBins); // fb_alloc(hist.LBinCount * sizeof(float), FB_ALLOC_NO_HINT);
