...
static void os04a10_linear_1080p60_12BIT_init(VI_PIPE ViPipe);
...
void os04a10_init(VI_PIPE ViPipe)
{
	...
	printf("OS04A10_DEBUG: os04a10_init mode=%d wdr=%d force_file=%d\n",
		u8ImgMode, enWDRMode, access("/tmp/force_1080p60", F_OK) == 0 ? 1 : 0);
	...
	if (enWDRMode == WDR_MODE_2To1_LINE) {
		...
		else if (u8ImgMode == OS04A10_MODE_1080P60_12BIT)
			os04a10_linear_1080p60_12BIT_init(ViPipe);
	}
...
static void os04a10_linear_1080p60_12BIT_init(VI_PIPE ViPipe)
{
	...
	os04a10_write_register(ViPipe, 0x37da, 0x00);
	os04a10_write_register(ViPipe, 0x37db, 0x00);
	os04a10_write_register(ViPipe, 0x3800, 0x01);	// crop x start MSB
	...
	os04a10_write_register(ViPipe, 0x380b, 0x38);	// output height LSB = 1080 = 0x438
	os04a10_write_register(ViPipe, 0x380c, 0x05);	// HTS MSB
	os04a10_write_register(ViPipe, 0x380d, 0xCC);	// HTS LSB = 1484 = 0x5CC
	os04a10_write_register(ViPipe, 0x380e, 0x04);	// VTS MSB
	os04a10_write_register(ViPipe, 0x380f, 0xC0);	// VTS LSB = 1216 = 0x4C0
	os04a10_write_register(ViPipe, 0x3811, 0x08);
	os04a10_write_register(ViPipe, 0x3813, 0x08);
	os04a10_write_register(ViPipe, 0x3814, 0x01);
	os04a10_write_register(ViPipe, 0x3815, 0x01);
	...
	printf("ViPipe:%d,===OS04A10 1080P 60fps 12bit LINE Init OK!===\n", ViPipe);
}
