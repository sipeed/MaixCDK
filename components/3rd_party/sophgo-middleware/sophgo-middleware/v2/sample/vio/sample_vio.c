...
#define VIO_FORMAT SAMPLE_PIXEL_FORMAT // PIXEL_FORMAT_RGB_888 or SAMPLE_PIXEL_FORMAT
...
CVI_S32 SAMPLE_PLAT_VO_INIT2(void)
{
	...
	stVoConfig.enPixFormat	 = VIO_FORMAT;
...
CVI_S32 SAMPLE_PLAT_VPSS_INIT2(VPSS_GRP VpssGrp, SIZE_S stSizeIn, SIZE_S stSizeOut)
{
	...
	astVpssChnAttr[VpssChn].enPixelFormat               = VIO_FORMAT;
...
CVI_S32 SAMPLE_PLAT_VPSS_INIT3(VPSS_GRP VpssGrp, SIZE_S stSizeIn, SIZE_S stSizeOut)
{
	VPSS_GRP_ATTR_S    stVpssGrpAttr;
	VPSS_CHN           VpssChn        = VPSS_CHN0;
	CVI_BOOL           abChnEnable[VPSS_MAX_PHY_CHN_NUM] = {0};
	...
	stVpssGrpAttr.stFrameRate.s32SrcFrameRate    = -1;
	stVpssGrpAttr.stFrameRate.s32DstFrameRate    = -1;
	stVpssGrpAttr.enPixelFormat                  = VIO_FORMAT;
	stVpssGrpAttr.u32MaxW                        = stSizeIn.u32Width;
	stVpssGrpAttr.u32MaxH                        = stSizeIn.u32Height;
	stVpssGrpAttr.u8VpssDev                      = 0;
...
static void nv21_scale(uint8_t *src, int src_w, int src_h, uint8_t *dst, int dst_w, int dst_h)
{
#if 0
	int i, j;
	int y, x;
	int scale_y = (src_h << 16) / dst_h;
	int scale_x = (src_w << 16) / dst_w;
	int y_pos, x_pos;
	uint8_t *src_y = src;
	uint8_t *dst_y = dst;
	uint8_t *src_uv = src + src_w * src_h;
	...
	for (dst_y = 0; dst_y < dst_h; dst_y++) {
		...
		for (dst_x = 0; dst_x < dst_w; dst_x++) {
			src_x = dst_x * scale_x;
			uint8_t y = src[src_x + src_w * src_y];
			uint8_t u = src[src_w * src_h + (src_x / 2) + (src_y / 2) * (src_w / 2)];
			uint8_t v = src[src_w * src_h + (src_x / 2) + (src_y / 2) * (src_w / 2) + 1];

			dst[dst_x + dst_y * dst_w] = y;
			if (dst_y % 2 == 0 && dst_x % 2 == 0) {
				dst[dst_w * dst_h + (dst_x / 2) + (dst_y / 2) * (dst_w / 2) + 1] = u;
				dst[dst_w * dst_h + (dst_x / 2) + (dst_y / 2) * (dst_w / 2)] = v;
			}
		}
	}
#endif
...
// static void dump_cvi_img(IVE_IMAGE_S *img)
// {
// 	printf("====== DUMP IMAGE ======\n");
// 	printf("u64PhyAddr[0]: 0x%lx\n", img->u64PhyAddr[0]);
// 	printf("u64PhyAddr[1]: 0x%lx\n", img->u64PhyAddr[1]);
// 	printf("u64PhyAddr[2]: 0x%lx\n", img->u64PhyAddr[2]);
// 	printf("u64VirAddr[0]: 0x%lx\n", img->u64VirAddr[0]);
// 	printf("u64VirAddr[1]: 0x%lx\n", img->u64VirAddr[1]);
// 	printf("u64VirAddr[2]: 0x%lx\n", img->u64VirAddr[2]);
// 	printf("u32Width: %d\n", img->u32Width);
// 	printf("u32Height: %d\n", img->u32Height);
...
#if 0		/*TEST_VO_ONLY*/
{
	...
	uint32_t filelen;
	if (load_file_to_dyna_mem("./640_480.rgb", &filebuf, &filelen) == 0) {
		printf("load_file_to_dyna_mem failed!\r\n");
		return -1;
	}

	mmf_init();

	int img_w = 640, img_h = 480, img_fmt = PIXEL_FORMAT_RGB_888;	//PIXEL_FORMAT_NV21 or PIXEL_FORMAT_RGB_888

	int layer = 0;
	int vo_ch = mmf_get_vo_unused_channel(layer);
	mmf_add_vo_channel(layer, vo_ch, img_w, img_h, img_fmt);
	...
	while (!exit_flag) {
		mmf_vo_frame_push(layer, vo_ch, filebuf, filelen, img_w, img_h, img_fmt);
		usleep(100 * 1000);
	}
...
#if 0
{
	...
	// uint8_t *rgb888 = malloc(img_w * img_h * 3);
	// if (rgb888 == NULL) {
	// 	printf("malloc failed!\r\n");
	// 	return -1;
	// }
	// uint8_t *rgb565 = malloc(img_w * img_h * 2);
	// if (rgb565 == NULL) {
	// 	printf("malloc failed!\r\n");
	// 	free(rgb888);
	// 	return -1;
	...
	// 	rgb565[i * 2] = (rgb888[i * 3] & 0xF8) | (rgb888[i * 3 + 1] >> 5);
	// 	rgb565[i * 2 + 1] = ((rgb888[i * 3 + 1] & 0x1C) << 3) | (rgb888[i * 3 + 2] >> 3);
	// }
	// gettimeofday(&tv, NULL);
	// end = tv.tv_usec + tv.tv_sec * 1000000;
	// printf("rgb888 to rgb565 time: %ld\n", end - start);
	// if (rgb888)
	// 	free(rgb888);
	// if (rgb565)
	// 	free(rgb565);
	uint8_t *file_buff;
	uint32_t file_size;
	load_file_to_dyna_mem("./640_480.rgb", &file_buff, &file_size);
	save_buff_to_file("640_480_csc.rgb", file_buff, file_size);
	system("sync");
	...
	if (img_w % 16 != 0 || img_w > 1920 || img_h % 16 != 0 || img_h > 1080) {
		printf("img_w and img_h must be multiple of 16, img_w must less than 1920, img_h must less than 1080!\r\n");
		CVI_IVE_DestroyHandle(handle);
		return -1;
	}

	IVE_IMAGE_S src_img, dst_img;
	if (0 != CVI_IVE_CreateImage(handle, &src_img, IVE_IMAGE_TYPE_U8C3_PACKAGE, img_w, img_h)) {
		printf("CVI_IVE_CreateImageInfo failed!\r\n");
		CVI_IVE_DestroyHandle(handle);
		return -1;
	}
	src_img.u32Stride[0] = 640;
	dump_cvi_img(&src_img);
	memcpy((uint8_t *)src_img.u64VirAddr[0], file_buff, img_w * img_h * 3 / 2);

	if (0 != CVI_IVE_CreateImage(handle, &dst_img, IVE_IMAGE_TYPE_YUV420SP, img_w, img_h)) {
		printf("CVI_IVE_CreateImageInfo failed!\r\n");
		CVI_IVE_DestroyHandle(handle);
		return -1;
	}
	dump_cvi_img(&dst_img);
	...
	gettimeofday(&tv, NULL);
	start = tv.tv_usec + tv.tv_sec * 1000000;
	if (0 != CVI_IVE_CSC(handle, &src_img, &dst_img, &stCtrl, 1)) {
		printf("CVI_IVE_CSC failed!\r\n");
		CVI_IVE_DestroyHandle(handle);
		return -1;
	}
	gettimeofday(&tv, NULL);
	printf("CVI_IVE_CSC time: %ld\n", tv.tv_usec + tv.tv_sec * 1000000 - start);


	save_buff_to_file("640_480_csc.yuv", (uint8_t *)dst_img.u64VirAddr[0], img_w * img_h * 3);

	// IVE_DST_DATA_S dst_data = {
	// 	.u32Width = dst_img.u32Width,
	// 	.u32Height = dst_img.u32Height,
	// 	.u32Stride = dst_img.u32Stride[0],
	// 	.u64PhyAddr = dst_img.u64PhyAddr[0],
	// };
	// CVI_SYS_FreeD(handle, &dst_data);


	free(file_buff);
	CVI_SYS_FreeI(handle, &src_img);
	CVI_SYS_FreeI(handle, &dst_img);
	CVI_IVE_DestroyHandle(handle);
	return 0;
}
#endif
	...
	while (!exit_flag) {
		...
		if (0 == mmf_vi_frame_pop(vi_ch, &data, &data_size, &width, &height, &format)) {
			...
			gettimeofday(&tv, NULL);
			SAMPLE_PRT(">>>>>> mmcpy vi frame %ld\n", tv.tv_usec / 1000 + tv.tv_sec * 1000);
			printf("Push..width:%d height:%d data_size:%d format:%d\r\n", img_w, img_h, show_img_size, img_fmt);
			mmf_vo_frame_push(layer, vo_ch, show_img, show_img_size, img_w, img_h, format);

			gettimeofday(&tv, NULL);
			SAMPLE_PRT(">>>>>> flush vo frame %ld\n", tv.tv_usec / 1000 + tv.tv_sec * 1000);

			UNUSED(nv21_scale);
			// int nv21_scale_w = 200;
			// int nv21_scale_h = 200;
			// int nv21_scale_size = nv21_scale_w * nv21_scale_h * 3 / 2;
			// uint8_t *nv21_scale_img = malloc(nv21_scale_size);
			// nv21_scale(data, width, height, nv21_scale_img, nv21_scale_w, nv21_scale_h);
			// save_buff_to_file("snapshot.yuv", data, data_size);
			// save_buff_to_file("snapshot_scale.yuv", nv21_scale_img, nv21_scale_size);
			// free(nv21_scale_img);
			// char path[128] = {0};
			// snprintf(path, sizeof(path), "./snapshot_%d.yuv", key);
			// save_buff_to_file("snapshot.yuv", show_img, show_img_size);
			// // snprintf(path, sizeof(path), "./snapshot_%d.rgb", key);
			// save_buff_to_file(path, data, data_size);
			// printf("Save to %s\r\n", path);
			// system("sync");
	...
	s32Ret = SAMPLE_PLAT_VPSS_INIT2(0, stSizeIn, stSizeOut);
	if (s32Ret != CVI_SUCCESS) {
	...
	s32Ret = SAMPLE_PLAT_VPSS_INIT3(1, stSizeOut, stSizeOut);
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_PRT("vpss init failed. s32Ret: 0x%x !\n", s32Ret);
		CVI_BOOL  abChnEnable[VPSS_MAX_PHY_CHN_NUM] = {0};
		abChnEnable[0] = CVI_TRUE;
	...
	// if (s32Ret != CVI_SUCCESS) {
	// 	SAMPLE_PRT("vpss bind vo failed. s32Ret: 0x%x !\n", s32Ret);
	// 	return s32Ret;
	// }
...
#if 1
	// int width = 1280, height = 720, format = SAMPLE_PIXEL_FORMAT;
	...
	// // while (!exit_flag) {
	// 	printf("Flush..width:%d height:%d format:%d\r\n", width, height, format);
	// 	mmf_vo_frame_push(0, 0, data, data_size, width, height, format);
	// // }
	// PAUSE();
#else
	CVI_U32 chnID = 0;
	do {
		...
		if (CVI_VPSS_GetChnFrame(0, 0, &frame, 3000) == 0) {
			printf("Get frame OK\n");
			CVI_VO_SendFrame(0, 0, &frame, 1000);

			// frame.stVFrame.u32Stride[0] = 1280;
			extern void mmf_dump_frame(VIDEO_FRAME_INFO_S *frame);
			mmf_dump_frame(&frame);

			// s32Ret = CVI_VPSS_SendChnFrame(1, 0, &frame, 1000);
			// if (s32Ret != CVI_SUCCESS) {
			// 	printf("CVI_VPSS_SendChnFrame NG, ret:%#x\n", s32Ret);
			// 	exit_flag = 1;
			// } else {
			// 	printf("CVI_VPSS_SendChnFrame OK\n");
			// 	VIDEO_FRAME_INFO_S frame2;
			// 	s32Ret = CVI_VPSS_GetChnFrame(1, 0, &frame2, 3000);
			// 	if (s32Ret == 0) {
			// 		printf("Get Group1 frame OK\n");
			// 		extern void mmf_dump_frame(VIDEO_FRAME_INFO_S *frame);
			// 		mmf_dump_frame(&frame2);
			// 		CVI_VPSS_ReleaseChnFrame(1, 0, &frame2);
			// 	} else {
			// 		printf("Get Group1 frame NG, ret:%#x\n", s32Ret);
			// 	}
			// }
...
CVI_S32 SAMPLE_VI_VPSS_SLT(CVI_S32 frmNum, char *filename)
{
	...
	do {
		...
		u32LumaSize =  stVideoFrame.stVFrame.u32Stride[0] * stVideoFrame.stVFrame.u32Height;
		if (stVideoFrame.stVFrame.enPixelFormat == PIXEL_FORMAT_RGB_888)
...
CVI_S32 SAMPLE_VIO_ViVpssAspectRatio(void)
{
	...
	do {
		...
		scanf("%d", &chnID);
		VIDEO_FRAME_INFO_S frame;
		if (CVI_VPSS_GetChnFrame(VpssGrp, VpssChn, &frame, 3000) == 0) {
			printf("Get frame OK\n");
			CVI_VO_SendFrame(0, 0, &frame, 1000);
			frame.stVFrame.u32Stride[0] = 1280;
			extern void mmf_dump_frame(VIDEO_FRAME_INFO_S *frame);
			mmf_dump_frame(&frame);
			CVI_VPSS_ReleaseChnFrame(VpssGrp, VpssChn, &frame);
		}
...
VO_UT_FILE vo_ut_file[] = {
	//0:test.bin（720 x 1280 x PIXEL_FORMAT_RGB_888_PLANAR）
	{
		.stSize.u32Width = 640,
		.stSize.u32Height = 480,
		.filename = "640_480.rgb",
		.enPixelFormat = PIXEL_FORMAT_RGB_888,
	},
	{
		.stSize.u32Width = 640,
		.stSize.u32Height = 480,
		.filename = "640_480.yuv",
		.enPixelFormat = PIXEL_FORMAT_NV21,
	},
};

static CVI_S32 sys_vb_init(void)
{
	CVI_S32 s32Ret;
	VB_CONFIG_S	stVbConf;
	...
	memset(&stVbConf, 0, sizeof(VB_CONFIG_S));
	stVbConf.u32MaxPoolCnt = 1;
	stVbConf.astCommPool[0].u32BlkSize = 0x600000; // 3M
	stVbConf.astCommPool[0].u32BlkCnt = 3;
...
// static CVI_S32 VO_PrepareFrame(SIZE_S stSize, PIXEL_FORMAT_E enPixelFormat, VIDEO_FRAME_INFO_S *pstVideoFrame)
// {
// 	VB_BLK blk;
...
// static CVI_S32 vo_send_frame(VO_UT_FILE *fileptr)
// {
// 	CVI_S32 ret = 0;
// 	FILE *fp;
// 	SIZE_S stSize;
...
// 		cfg.stVideoFrame.stVFrame.u64PhyAddr[0],
// 		cfg.stVideoFrame.stVFrame.u64PhyAddr[1],
// 		cfg.stVideoFrame.stVFrame.u64PhyAddr[2]);

// 	if (cfg.stVideoFrame.stVFrame.u64PhyAddr[1]) {
...
// 	if (cfg.stVideoFrame.stVFrame.u64PhyAddr[2]) {
// 		cfg.stVideoFrame.stVFrame.pu8VirAddr[2]
// 			= CVI_SYS_Mmap(cfg.stVideoFrame.stVFrame.u64PhyAddr[2], cfg.stVideoFrame.stVFrame.u32Length[2]);
// 		if (cfg.stVideoFrame.stVFrame.pu8VirAddr[2] == NULL) {
// 			SAMPLE_PRT("CVI_SYS_Mmap failed\n");
// 			CVI_SYS_Munmap(cfg.stVideoFrame.stVFrame.pu8VirAddr[0], cfg.stVideoFrame.stVFrame.u32Length[0]);
...
// 	CVI_SYS_Munmap(cfg.stVideoFrame.stVFrame.pu8VirAddr[0], cfg.stVideoFrame.stVFrame.u32Length[0]);
// 	CVI_SYS_Munmap(cfg.stVideoFrame.stVFrame.pu8VirAddr[1], cfg.stVideoFrame.stVFrame.u32Length[1]);
// 	CVI_SYS_Munmap(cfg.stVideoFrame.stVFrame.pu8VirAddr[2], cfg.stVideoFrame.stVFrame.u32Length[2]);

// 	CVI_VO_SendFrame(0, 0, &cfg.stVideoFrame, -1);

// 	CVI_VB_ReleaseBlock(CVI_VB_PhysAddr2Handle(cfg.stVideoFrame.stVFrame.u64PhyAddr[0]));
// 	return ret;
// }

CVI_S32 SAMPLE_VO_RGB888P_TEST(CVI_VOID)
{
#if 1
{
	int file_id = 0;
	SAMPLE_VO_CONFIG_S stVoConfig;
	RECT_S stDefDispRect  = {0, 0, 640, 480};
	UNUSED(stDefDispRect);
	UNUSED(stVoConfig);
	CVI_S32 s32Ret = CVI_SUCCESS;
	signal(SIGINT, sig_handle);
	signal(SIGTERM, sig_handle);
	...
	stVoConfig.stVoPubAttr.enIntfSync  = VO_OUTPUT_720x1280_60;
	stVoConfig.stDispRect	 = stDefDispRect;
	stVoConfig.stImageSize	 = vo_ut_file[file_id].stSize;
	stVoConfig.enPixFormat	 = vo_ut_file[file_id].enPixelFormat;
	stVoConfig.enVoMode	 = VO_MODE_1MUX;

	SAMPLE_PRT("stDefDispRect Width:%d Height:%d\n", stDefDispRect.u32Width, stDefDispRect.u32Height);
	SAMPLE_PRT("stImageSize Width:%d Height:%d\n", vo_ut_file[file_id].stSize.u32Width, vo_ut_file[file_id].stSize.u32Height);
	SAMPLE_PRT("SAMPLE_COMM_VO_StartVO fmt %x\n", stVoConfig.enPixFormat);
	s32Ret = SAMPLE_COMM_VO_StartVO(&stVoConfig);
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_PRT("SAMPLE_COMM_VO_StartVO failed with %#x\n", s32Ret);
		goto error2;
	...
	// s32Ret = CVI_VO_SetChnRotation(0, 0, ROTATION_90);
	// if (s32Ret != CVI_SUCCESS) {
	// 	SAMPLE_PRT("CVI_VO_SetChnRotation failed with %#x\n", s32Ret);
	// 	goto error2;
	// }
	...
	while (!exit_flag) {
		...
		s32Ret = vo_send_frame(&vo_ut_file[file_id]);
		if (s32Ret != CVI_SUCCESS) {
			SAMPLE_PRT("vo_send_frame failed with %#x!\n", s32Ret);
			break;
		}

		gettimeofday(&tv, NULL);
		end = tv.tv_usec + tv.tv_sec * 1000000;
		SAMPLE_PRT(">>>>>> vo_send_frame time %ld\n", (end - start));
		usleep(30*1000);
	}
	...
	int layer = 0;
	int vo_ch = mmf_get_vo_unused_channel(layer);
	int res = mmf_add_vo_channel(layer, vo_ch,
						vo_ut_file[file_id].stSize.u32Width,
						vo_ut_file[file_id].stSize.u32Height,
						// vo_ut_file[file_id].stSize.u32Height,
						// vo_ut_file[file_id].stSize.u32Width,
						vo_ut_file[file_id].enPixelFormat);
	if (res != 0) {
		SAMPLE_PRT("mmf_add_vo_channel failed with %#x\n", res);
		goto error2;
	}

	load_file_to_dyna_mem(vo_ut_file[0].filename, &pic_buff, &pic_buff_len);
	...
	while (!exit_flag) {
		...
		mmf_vo_frame_push(layer, vo_ch,
						pic_buff, pic_buff_len,
						vo_ut_file[file_id].stSize.u32Width,
						vo_ut_file[file_id].stSize.u32Height,
						// vo_ut_file[file_id].stSize.u32Height,
						// vo_ut_file[file_id].stSize.u32Width,
...
error2:
	...
	s32Ret = sys_vb_init();
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_PRT("sys_vb_init failed\n");
		return CVI_FAILURE;
	}

	CVI_VO_Get_Panel_Status(0, 0, &panel_init);
	SAMPLE_PRT("vo_ut_plat_vo_init[%d]\n", panel_init);
	...
	stVoConfig.stVoPubAttr.enIntfSync  = VO_OUTPUT_720x1280_60;
	stVoConfig.stDispRect	 = stDefDispRect;
	stVoConfig.stImageSize	 = vo_ut_file[0].stSize;
	stVoConfig.enPixFormat	 = vo_ut_file[0].enPixelFormat;
	stVoConfig.enVoMode	 = VO_MODE_1MUX;
	...
	for (int i = 0; i < 10; i++) {
		// s32Ret = vo_send_frame(&vo_ut_file[0]);
		if (s32Ret != CVI_SUCCESS) {
			SAMPLE_PRT("vo_send_frame failed with %#x!\n", s32Ret);
			break;
		}
