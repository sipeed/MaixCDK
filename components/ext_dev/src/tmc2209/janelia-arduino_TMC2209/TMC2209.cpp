...
// #include "tmc2209_debug.hpp"
#include <mutex>

static void tmc2209_sleep_ms(const uint32_t ms)
{
  maix::time::sleep_ms(ms);
}

long map(long num, long fromLow, long fromHigh, long toLow, long toHigh)
{
...
// }
void TMC2209::setup(const char* port, long serial_baud_rate,
    SerialAddress serial_address)
{
  // this->_uart = std::make_unique<maix::peripheral::uart::UART>(
  //   std::string(port), static_cast<int>(serial_baud_rate));
  this->_uart = std::unique_ptr<maix::peripheral::uart::UART>(
    new maix::peripheral::uart::UART(std::string(port), static_cast<int>(serial_baud_rate)));
  this->initialize(serial_baud_rate, serial_address);
}
#if defined(ESP32)
void TMC2209::setup(HardwareSerial & serial,
  long serial_baud_rate,
...
void TMC2209::setMicrostepsPerStep(uint16_t microsteps_per_step)
{
  ...
  // tmc2209_debug::info<false>("[setMicrostepsPerStep] exponent = %lu", exponent);
...
bool TMC2209::hardwareDisabled()
{
  ...
  // maix::log::info("[hardwareDisabled] 0x%04x", input.bytes);
...
std::size_t TMC2209::serialWrite(uint8_t c)
{
  // tmc2209_debug::info_byte<true>(c);
  this->_uart->write(&c, 1);
  // if (hardware_serial_ptr_ != nullptr)
  // {
...
// constexpr uint8_t buffer_len = 32;
// static uint8_t next_index = 0;
// static uint8_t read_index = 0;
// static uint8_t buffer[buffer_len];
// static std::once_flag _buffer_init_flag;

int TMC2209::serialRead()
{
  // uint8_t buff[32];
  // ::memset(buff, 0x00, sizeof(buff));
  // int num = this->_uart->read(buff, sizeof(buff));
  // if (num > 0) {
  //   tmc2209_debug::info<>("[%s:%d] Read all datas", __PRETTY_FUNCTION__, __LINE__);
  //   for (int i = 0; i < num; ++i) {
  //     tmc2209_debug::info_byte<true>(buff[i]);
  //   }
  //   tmc2209_debug::info<>("[%s:%d] Read end...", __PRETTY_FUNCTION__, __LINE__);
  // }

  // return static_cast<int>(0x00);

  uint8_t data = 0x00;
  this->_uart->read(&data, 1, 1);
  return static_cast<int>(data);

  // std::call_once(_buffer_init_flag, [](){
  //   tmc2209_debug::info<true>("[%s:%d] Init read buffer.", __PRETTY_FUNCTION__, __LINE__);
  //   ::memset(buffer, 0x00, buffer_len);
  // });

  // int num = this->_uart->available();
  // int free_buffer_len = buffer_len - next_index;
  // tmc2209_debug::info<>("[%s:%d] %d bytes available, %d bytes free",
  //   __PRETTY_FUNCTION__, __LINE__, num, free_buffer_len);
  // if (free_buffer_len >= num) {
  //   //push data to buffer
  //   tmc2209_debug::info<>("[%s:%d] Push datas into buffer...", __PRETTY_FUNCTION__, __LINE__);
...
void TMC2209::sendDatagramUnidirectional(Datagram & datagram,
  uint8_t datagram_size)
{
  // tmc2209_debug::info<false>("[%s:%d] Send bytes:", __PRETTY_FUNCTION__, __LINE__);
  uint8_t byte;
  ...
  for (uint8_t i=0; i<datagram_size; ++i)
  {
    byte = (datagram.bytes >> (i * BITS_PER_BYTE)) & BYTE_MAX_VALUE;
    serialWrite(byte);
    // tmc2209_debug::info_byte<false>(byte);
  }

  static constexpr uint64_t TIMEOUT = 5;
  uint64_t start_time = maix::time::ticks_ms();
  // tmc2209_debug::info<false>("[%s:%d] [%llu] Wait %d bytes to clean...", __PRETTY_FUNCTION__, __LINE__, start_time, datagram_size);
  while (this->serialAvailable() < datagram_size && maix::time::ticks_ms()-start_time<TIMEOUT) {
    maix::time::sleep_ms(1);
  }
  if (this->serialAvailable() < datagram_size) {
    // tmc2209_debug::info<false>("[%s:%d] [%llu] Wait %d bytes failed",
    //         __PRETTY_FUNCTION__, __LINE__, maix::time::ticks_ms(), datagram_size);
    return;
  }
  // tmc2209_debug::info<false>("[%s:%d] [used time: %llu ms] Wait %d bytes succ",
  //           __PRETTY_FUNCTION__, __LINE__, maix::time::ticks_ms()-start_time, datagram_size);
  // tmc2209_debug::info<false>("[%s:%d] Clear bytes:", __PRETTY_FUNCTION__, __LINE__);
  for (int i = 0; i < datagram_size; ++i) {
    /*[[maybe_unsed]]auto byte = */this->serialRead();
    // tmc2209_debug::info_byte<true>(static_cast<uint8_t>(byte));
  }
  // tmc2209_debug::info<false>("\n\n");
}
  ...
  uint8_t datagram_size)
{
    ...
    (echo_delay < ECHO_DELAY_MAX_MICROSECONDS))
  {
    ...
    tmc2209_sleep_ms(ECHO_DELAY_INC_MICROSECONDS);
...
{
  // tmc2209_debug::info<false>("[%s:%d]", __PRETTY_FUNCTION__, __LINE__);
  ...
  //   tmc2209_sleep_ms(REPLY_DELAY_INC_MICROSECONDS);
  ...
  for (uint8_t i=0; i<WRITE_READ_REPLY_DATAGRAM_SIZE; ++i)
  {
    ...
    // tmc2209_debug::info<true>("[%s:%d] Read bytes : 0x%x", __PRETTY_FUNCTION__, __LINE__, byte);
  }
