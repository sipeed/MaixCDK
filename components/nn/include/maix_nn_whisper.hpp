...
namespace maix::nn
{
    ...
    class Whisper
    {
        nn::NN *_encoder_model;
        nn::NN *_decoder_main_model;
        nn::NN *_decoder_loop_model;
        std::vector<float> _positional_embedding;
        std::vector<std::string> _token_tables;
        std::string _language;
        std::string _whisper_type;
        ...
        std::unique_ptr<opencc::SimpleConverter> _simple_converter;
    protected:
    ...
    public:
        ...
        Whisper(const string &model = "", std::string language = "zh") {
            _encoder_model = nullptr;
            _decoder_main_model = nullptr;
            _decoder_loop_model = nullptr;
            _dual_buff = false;
            _language = language;
        ...
        */
        err::Err unload() {
            if (_encoder_model) {
                delete _encoder_model;
                _encoder_model = NULL;
            }

            if (_decoder_main_model) {
                delete _decoder_main_model;
                _decoder_main_model = NULL;
            }

           if (_decoder_loop_model) {
                delete _decoder_loop_model;
                _decoder_loop_model = NULL;
           }
           return err::ERR_NONE;
        }
        ...
        err::Err load(const string &model) {
            ...
            _encoder_model = new nn::NN(model, _dual_buff);
            if (!_encoder_model) {
                return err::ERR_NO_MEM;
            }

            _extra_info = _encoder_model->extra_info();
            ...
            res = __load_value_from_map(_extra_info, "input_type", value_string);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            ...
            res = __load_value_from_map(_extra_info, "pcm_samplerate", value_int);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            ...
            // res = __load_value_from_map(_extra_info, "pcm_channels", value_int);
            // if (res != err::ERR_NONE) {
            //     this->unload();
            //     return res;
            // }
            ...
            // res = __load_value_from_map(_extra_info, "pcm_bits_per_frame", value_int);
            // if (res != err::ERR_NONE) {
            //     this->unload();
            //     return res;
            // }
            ...
            res = __load_value_from_map(_extra_info, "n_fft", value_int);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            ...
            res = __load_value_from_map(_extra_info, "n_hop", value_int);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            ...
            res = __load_value_from_map(_extra_info, "n_mels", value_int);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            ...
            res = __load_value_from_map(_extra_info, "whisper_type", value_string);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            _whisper_type = value_string;

            // load decoder-main model
            res = __load_value_from_map(_extra_info, "decoder-main", value_string);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            std::string model_file = fs::dirname(model) + "/" + value_string;
            _decoder_main_model = new nn::NN(model_file);
            if (_decoder_main_model == nullptr) {
            ...
            res = __load_value_from_map(_extra_info, "decoder-loop", value_string);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }
            model_file = fs::dirname(model) + "/" + value_string;
            _decoder_loop_model = new nn::NN(model_file);
            if (_decoder_loop_model == nullptr) {
            ...
            // load pe
            res = __load_value_from_map(_extra_info, "positional_embedding", value_string);
            if (res != err::ERR_NONE) {
                this->unload();
                return res;
            }

            if (_whisper_type == "tiny") {
                _n_text_state = 384;
            } else if (_whisper_type == "base") {
                _n_text_state = 512;
            } else if (_whisper_type == "small") {
                _n_text_state = 768;
            } else {
                log::error("invalid whisper type:%s", _whisper_type.c_str());
                this->unload();
                return err::ERR_ARGS;
            }
            _positional_embedding.resize(WHISPER_N_TEXT_CTX * _n_text_state);
            ...
            fread(_positional_embedding.data(), sizeof(float), WHISPER_N_TEXT_CTX * _n_text_state, fp);
            fclose(fp);
            ...
            while (std::getline(ifs, line)) {
                size_t i = line.find(' ');
                _token_tables.push_back(line.substr(0, i));
            }

            // load tokens
            _simple_converter = std::make_unique<opencc::SimpleConverter>("/opt/etc/t2s.json");
            return err::ERR_NONE;
        }
        ...
        */
        std::string transcribe_raw(Bytes *pcm, int sample_rate = 16000, int channels = 1, int bits_per_frame = 16) {
            ...
            auto mel = librosa_simple::Feature::melspectrogram(pcm_data, _input_pcm_samplerate, _n_fft, _n_hop, "hann", true, "reflect", 2.0f, _n_mels, 0.0f, _input_pcm_samplerate / 2);
            int n_mel = mel.size();
            ...
            auto encoder_outputs_info0 = _encoder_model->outputs_info()[0];
            auto encoder_outputs_info1 = _encoder_model->outputs_info()[1];
            ...
            auto decoder_main_outputs_info1 = _decoder_main_model->outputs_info()[1];
            auto decoder_main_outputs_info2 = _decoder_main_model->outputs_info()[2];
            ...
            if ( err::ERR_NONE != (err = _encoder_model->forward(encoder_input_tensors, encoder_output_tensors, false, true))) {
                log::error("encoder forward failed! err:%d", err);
                return "";
            }

            SOT_SEQUENCE[1] = detect_language(_language);
            ...
            if (err::ERR_NONE != (err = _decoder_main_model->forward(decoder_main_input_tensors, decoder_main_output_tensors, false, true))) {
            ...
            int WHISPER_N_TEXT_STATE = WHISPER_N_TEXT_STATE_MAP[_whisper_type];
            ...
            for (size_t i = 0; i < WHISPER_N_TEXT_CTX - SOT_SEQUENCE.size(); i++) {
                ...
                auto decoder_loop_input_tensor5 = new tensor::Tensor({1, 512}, tensor::DType::FLOAT32, _positional_embedding.data() + offset * WHISPER_N_TEXT_STATE, false);
                ...
                if (err::ERR_NONE != (err = _decoder_loop_model->forward(decoder_loop_input_tensors, decoder_loop_output_tensors, false, true))) {
            ...
            for (const auto i : results) {
                char str[1024];
                base64_decode((const uint8_t*)_token_tables[i].c_str(), (uint32_t)_token_tables[i].size(), str);
                s += str;
            }

            if (_language == "zh") {
                s = _simple_converter->Convert(s);
            }
            return s;
        }
