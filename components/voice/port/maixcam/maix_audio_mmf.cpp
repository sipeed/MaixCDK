...
namespace maix::audio
{
    ...
    static int _alsa_capture_pause_init(  snd_pcm_t **handle, snd_pcm_hw_params_t **hw_params,
                                    snd_pcm_format_t format, unsigned int sample_rate, unsigned int channels)
    {
        int err = 0;
        if (handle) *handle = NULL;
    ...
    static int _alsa_capture_pause_deinit(snd_pcm_t *handle)
    {
        if (handle) {
            return snd_pcm_close(handle);
        }
    ...
    static int _alsa_get_old_frame_size(void)
    {
        return 512;
    }

    static void *_alsa_prepare_old_buffer(snd_pcm_format_t format, unsigned int channels, snd_pcm_uframes_t  *buffer_size)
    {
        int buffer_frames = _alsa_get_old_frame_size();
        int frame_byte = snd_pcm_format_width(format) / 8;
        snd_pcm_uframes_t new_buffer_size = buffer_frames * frame_byte * channels;
        void *buffer = malloc(new_buffer_size);
        if (!buffer) {
    ...
    static int _alsa_capture_pause_pop(snd_pcm_t *handle, snd_pcm_format_t format, unsigned int channels, void *buffer, snd_pcm_uframes_t buffer_size)
    {
        int len = 0;
        int frame_size = _alsa_get_old_frame_size();
        int frame_byte = snd_pcm_format_width(format) / 8;
        if (buffer_size < frame_byte * channels * frame_size) {
            printf("Bad buffer size, input %ld, need %d\r\n", buffer_size, frame_byte * channels * frame_size);
            return 0;
    ...
    static int _alsa_player_block_init(  snd_pcm_t **handle, snd_pcm_hw_params_t **hw_params,
                                    snd_pcm_format_t format, unsigned int sample_rate, unsigned int channels)
    {
        int err = 0;
    ...
    static int _alsa_player_block_deinit(snd_pcm_t *handle)
    {
        if (handle) {
            return snd_pcm_close(handle);
        }
    ...
    static int _alsa_player_block_push(snd_pcm_t *handle, snd_pcm_format_t format, unsigned int channels, void *buffer, snd_pcm_uframes_t buffer_size)
    {
        int len = 0;
        int frame_byte = snd_pcm_format_width(format) / 8;
        int write_count = buffer_size  / frame_byte / channels;
    ...
    static bool first_init_old = true;
    static void _trigger_segmentation_fault_old(void) {
        pid_t pid;
        int status;
        pid = fork();
        if (pid == -1) {
            ...
            snd_pcm_t *handle;
            snd_pcm_hw_params_t *hwparams;
            _alsa_capture_pause_init(&handle, &hwparams, SND_PCM_FORMAT_S16_LE, 48000, 1);
            _alsa_capture_pause_pop(handle, SND_PCM_FORMAT_S16_LE, 1, buffer, sizeof(buffer));
            _alsa_capture_pause_deinit(handle);
            exit(EXIT_SUCCESS);
        } else {
            waitpid(pid, &status, 0);
        }
    ...
    static void _fix_segmentation_fault_bug_old(void) {
        if (!first_init_old) {
            return;
        }

        for (int i = 0; i < 3; i ++) {
            _trigger_segmentation_fault_old();
        }

        first_init_old = false;
    }


    RecorderPause::RecorderPause(std::string path, int sample_rate, audio::Format format, int channel) {
        _path = path;
        _sample_rate = sample_rate;
        _format = format;
        _channel = channel;
        ...
        snd_pcm_hw_params_t *hwparams;
        snd_pcm_format_t format_p = _alsa_format_from_maix(format);
        _fix_segmentation_fault_bug_old();
        err::check_bool_raise(0 <= _alsa_capture_pause_init(&handle, &hwparams, format_p, sample_rate, channel), "capture init failed");
        _handle = handle;
        snd_pcm_uframes_t buffer_size = 0;
        _buffer = _alsa_prepare_old_buffer(format_p, channel, &buffer_size);
        err::check_null_raise(_buffer, "record buffer init failed!");
        _buffer_size = (size_t)buffer_size;
    ...
    RecorderPause::~RecorderPause() {
        if (_handle) {
            snd_pcm_drain((snd_pcm_t *)_handle);
            err::check_bool_raise(0 <= _alsa_capture_pause_deinit((snd_pcm_t *)_handle));
            _handle = NULL;
        }
    ...
    int RecorderPause::volume(int value) {
        char buffer[512];
        if (value != -1) {
            snprintf(buffer, sizeof(buffer), "amixer -Dhw:0 cset name='ADC Capture Volume' %d &> /dev/zero", value);
            system(buffer);
    ...
    maix::Bytes *RecorderPause::record_pcm_to_file_keep_pause(std::string path_record_pcm_file,std::atomic<bool>& flag_record,std::atomic<bool>& flag_record_pause) 
    {
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
        void *buffer = _buffer;
        snd_pcm_uframes_t buffer_size = (snd_pcm_uframes_t)_buffer_size;
        ...
        if (flag_record.load()) 
        {
            ...
            while (flag_record.load()) 
            {
                ...
                len = 0;
                while (len >= 0) {
                    len = _alsa_capture_pause_pop(handle, format, channel, buffer, buffer_size);
                    if (len > 0) {
                        if (record_pcm_file)
                        {
    ...
    maix::Bytes *PlayerBlock::NoneBytes = new maix::Bytes();

    PlayerBlock::PlayerBlock(std::string path, int sample_rate, audio::Format format, int channel) 
    {
        _path = path;
        _sample_rate = sample_rate;
        _format = format;
        ...
        snd_pcm_format_t format_p = _alsa_format_from_maix(format);
        _fix_segmentation_fault_bug_old();
        err::check_bool_raise(0 <= _alsa_player_block_init(&handle, &hwparams, format_p, sample_rate, channel), "capture init failed");
        _handle = handle;
        snd_pcm_uframes_t buffer_size = 0;
        _buffer = _alsa_prepare_old_buffer(format_p, channel, &buffer_size);
        err::check_null_raise(_buffer, "player buffer init failed!");
    ...
    PlayerBlock::~PlayerBlock() {
        if (_handle) {
            snd_pcm_drain((snd_pcm_t *)_handle);
            err::check_bool_raise(0 <= _alsa_player_block_deinit((snd_pcm_t *)_handle));
            _handle = NULL;
        }
    ...
    int PlayerBlock::volume(int value) {
        char buffer[512];
    ...
    void PlayerBlock::audio_prepare() 
    {
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
        snd_pcm_prepare(handle);  // 准备设备

    }
    void PlayerBlock::audio_drain() 
    {
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
        snd_pcm_drain(handle);  // 准备设备

    }
    void PlayerBlock::audio_drop() 
    {
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
        snd_pcm_drop(handle);  // 准备设备

    }
    int PlayerBlock::audio_pause(int enable) 
    {
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
    ...
    err::Err PlayerBlock::play_data(maix::Bytes *data) 
    {
        err::Err ret = err::ERR_NONE;
        snd_pcm_t *handle = (snd_pcm_t *)_handle;
        snd_pcm_format_t format = _alsa_format_from_maix(_format);
        ...
        {
            len = _alsa_player_block_push(handle, format, channel, data->data, data->data_len);
            if (len < 0) {
                log::error("play failed, %s\r\n", snd_strerror(len));
                ret = err::ERR_RUNTIME;
            } else if ((size_t)len != data->data_len) {
